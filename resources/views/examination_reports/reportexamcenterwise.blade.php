					...
					<div class="row">
						<div class="col s12">
							<div class="card">
								<div class="card-content">
									<div class="row"> 
						<form action="{{route('reportexamcenterwise')}}" method="POST">
						@csrf
						<div class="row">
						<div class="col m2 s6" style=" margin-left: 83%;">
						{!! Form::select('pagecount',@$defaultOptions,@$pagecount, ['class' => 'select2 browser-default form-control center-align', 'onchange' => 'this.form.submit(); '] ) !!}
						 </div>
						   </div>
						</form>						
  <table >
	...
	@foreach ($master as $value)
		...
		@foreach ($value->examcenterallotments as $examcenterallotments1)
			...
			$is_stream_10 = 'is_student_strem'.$stream.'_10';
			$is_stream_12 = 'is_student_strem'.$stream.'_12';

			if($examcenterallotments1->$is_stream_10==1){
			$x=$examcenterallotments1->$stream_10;
			}elseif($examcenterallotments1->$is_stream_10==0){
			$x=0;
			}
      if($examcenterallotments1->$is_stream_12==1){
			$x1=$examcenterallotments1->$stream_12;  
			}elseif($examcenterallotments1->$is_stream_12==0){
			$x1=0;
			}
			...
			if($j==1){ @endphp 
				...
				<td><?php if($examcenterallotments1->$is_stream_10 ==1){ echo @$examcenterallotments1->$stream_10; }else{ echo '0'; }   ?></td>
				<td><?php if($examcenterallotments1->$is_stream_12 ==1){ echo @$examcenterallotments1->$stream_12;  }else{ echo '0'; }   ?> </td>
        <td><?php if($examcenterallotments1->is_student_supp_10 ==1){  echo @$examcenterallotments1->student_supp_10; }else{ echo '0'; }    ?></td>
				<td><?php if($examcenterallotments1->is_student_supp_12 ==1){   echo @$examcenterallotments1->student_supp_12; }else{ echo '0'; }   ?> </td>
				<td>{{ @$z }}</td>
				<td>{{ @$z2 }}</td>
				<td>{{ @$z3 }} </td>
				</tr>
