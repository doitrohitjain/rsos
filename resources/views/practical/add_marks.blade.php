			...
			<div class="container"> 
				<div class="section section-data-tables"> 
					<div class="row">
						...
						@php
						$note = null;
							if(isset($currentSlot) && !empty($currentSlot)){
								$note ="Note:practical slot timing ending at ".  date('d-m-Y H:i:s',strtotime($currentSlot->date_time_end)).'.' ."All remaning students will be mark 
								absent and will be visible in next slot or visible in  Button on home screen.";
							}
						@endphp
						<h6><a href="{{route('add_marks',$e_user_examiner_map_id)}}" class="btn btn-xs btn-info right mb-2 mr-1">Practical Marks</a></h6>
												...
												@foreach(@$master as $k=>$data)
													<tr>
														...
														<td>
															...
															?>
															<input type='checkbox' id="practicalAbsent<?php echo $i; ?>" name='data[{{ $k }}][practical_absent]' class="practical_absent practical_absent_<?php echo $i; ?> check_absent_marks" <?php echo $is_check_class; ?> value="{{@$data->id}}" /><span></span>
															</label>
														</td>
														<!-- <td>
														...
														<td>
															...
															?>
															<input type='text' id="finalPracticalMarks<?php echo @$data->id; ?>" name='data[{{ $k }}][final_practical_marks]' class="num final_practical_marks final_practical_marks_<?php echo $i; ?>  check_absent_marks" value="{{ @$final_practical_marks }}"   <?php if(@$data->practical_absent=='1'){ echo "readonly"; } ?>  maxlength="2" >
															
															<input type='hidden' name='data[{{ $k }}][student_allotment_marks_id]' class='student_allotment_marks_id' value='{{ ($data->id) }}'>
														</td>
													</tr>
												@php  $i++; @endphp
												@endforeach
