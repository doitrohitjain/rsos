					...
					<div class="seaction">
						...
						<div class="col s12 m12 l12">
							<div class="card-content">
								<?php  
									if($isAlreadyRaisedRequest > 0 && $student->update_supp_change_requests_challan_tid == "" ){ ?>
								<br/>
								<hr/>
								...
								style="" href="{{ route('supp_change_request_verify_request',Crypt::encrypt(@$student->student_id)) }}">
									Verify Your Payment
								</a> 
								...
								<?php if( $issueCount <= 0){ ?>
									...
									In case you want to raise a request to admin for your transaction got failed, please click on the raise a request button.
										...
										<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange"  href="{{ route('supp_change_request_raise_request',Crypt::encrypt(@$student->student_id)) }}">
							...
							<div id="Form-advance" class="card card card-default scrollspy">
								...
								<div class="card-content">
									...
									<div class="card-content">
										...
										if(@$student->update_supp_change_requests_challan_tid && $student->update_supp_change_requests_challan_tid != ""){ ?>
											<?php 
												if(@$student->locksumbitted && @$student->locksubmitted_date){ ?>
													<div class="">
														...
														<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange" 
														style ="" 
														href="{{ route('supp_generate_student_pdf',Crypt::encrypt(@$student->student_id)) }}">
									...
									<div class="row">
										...
										<div class="input-field col s4">
											...
											<h8>
												{!!Form::label($fld, $lbl) !!}
												{!! @$course[$studentsuppdata->$fld] !!}
											</h8>
										</div>
										<div class="input-field col s4">
											@php $lbl="Stream"; $fld='exam_month'; @endphp
											<h8>
												{!!Form::label($fld, $lbl) !!}
												{!! @$stream_id[$studentsuppdata->$fld] !!}
											</h8>
										</div>
									...
									<div class="row">
										...
										<div class="input-field col s8" style="font-size:16px;">
											@php 
												$lbl = __('Grand Total of Change Request Application Form filling <b><u> Fee Payment Amount</b></u>'); $lblhi = __('(आवेदन पत्र भरने का शुल्क <b><u>भुगतान राशि का कुल योग</b></u>)');
												
											@endphp 
											<span style="color:green;">
												{!! @$lbl . " : <span style='font-size:25px;''> Rs. " . @$changemakepayment  . "/-</span> <br>" .  $lblhi . " : <span style='font-size:25px;'>रुपये " . @$changemakepayment !!}/- </span>
											</span> 
										</div>
									</div> 
								</div> 
								...
								<!-- Fee payment button start -->
									<div class="card-content">
										...
										if($student->update_supp_change_requests_challan_tid == "" && $checkchangerequestsssupplementariesAllowOrNotAllow == true){
...
<script>
	...
	<script src="{!! asset('public/app-assets/js/bladejs/chang_request_supp_payment/registration_fee.js') !!}"></script> 
@endsection