									...
									<div class="card-content">
										...
										<table class="table"> 
											...
											<tr>
												...
												<td colspan="3">
												@php if(!empty($master->Address->address1)) { echo @$master->Address->address1.","; } @endphp
												@php if(!empty($master->Address->address2)) { echo @$master->Address->address2.","; } @endphp
												@php if(!empty($master->Address->address3)) { echo @$master->Address->address3.","; } @endphp
												@php if(!empty($master->Address->block_name)) { echo @$master->Address->block_name.","; } @endphp
												@php if(!empty($master->Address->tehsil_name)) { echo @$master->Address->tehsil_name.","; } @endphp
												@php if(!empty($master->Address->city_name)) { echo @$master->Address->city_name.","; } @endphp
												@php if(!empty($master->Address->district_name)) { echo @$master->Address->district_name.","; } @endphp
												@php if(!empty($master->Address->state_name)) { echo @$master->Address->state_name.",";  } @endphp
												</td>
											</tr>
										</table><br>
										<p style="color:red;font-size:16px;cursor:pointer;">Note:  Mobile Number Can Be Edited By Clicking On Mobile Number.</p>
										</div>
									</div>
											...
											<div id="Form-advance" class="card card card-default scrollspy">
												...
												{{ Form::open(['route' => [request()->route()->getAction()['as'], $estudent_id],'enctype' => 'multipart/form-data', 'id' => $model]) }}
													...
													<div class="row">
														...
														$comCounter = 0;
														foreach($supp_subject_arr as $k => $findComSubject){
															if(@$findComSubject['additional'] && @$findComSubject['additional'] == true){
														...
														/* gen condition start for general,subjct_cout 4 */ 
															if($subject_count == 4 && @$master->adm_type == 1){
																$genAdmSucjectCountFour = true;
																$subject_count = $subject_count + 1;
															}

															if($subject_count == 3 && @$master->adm_type == 1){
																$genAdmSucjectCountFour = true;
																$subject_count = $subject_count + 2;
															}
														/* gen condition end */
														$remaining_subject_count = 7 - $subject_count;
														$remaining_subject_count = 6 + $remaining_subject_count;
														...
														@php 
															if(@$master->adm_type == 5 && @$master->course == 10){
																$subject_count = 2;
															}
														@endphp
														...
														@php
														// if($k < 6 && @$supp_subject_arr[$k]['additional'] && !empty(@$supp_subject_arr[$k]['additional'])){
														// 	continue;
														// }
														
														if(@$supp_subject_arr[$k]['final_result'] != 'p' && @$supp_subject_arr[$k]['final_result'] != 'P' && @$supp_subject_arr[$k]['final_result'] != 'PASS'){  @endphp 
															<tr>
																<td> 
																<?php
																//asdfaf
																...
																if(!empty($subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']]) && $subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']]!=null ){
																	// echo "Test - " .@$supp_subject_arr[$k]['origional_subject_id'] . " ";
																	echo $subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']];
																} else {
																	echo "SELECT SUBJECT";
																}  
																echo '</span>';
																...
																if(@$master->adm_type == 5 && @$master->course == 12){
																	...
																	// $subject_list = array();
																	// $subject_list[18] = "Hindi (201)";
																	// $subject_list[19] = "English (202)";
																}else if(@$master->adm_type == 3){
																...
																<td>
																	@php $fld = 'origional_subject_id[]'; @endphp
																	<input type="hidden" value="{{ @$supp_subject_arr[$k]['origional_subject_id'] }}" name="{{ $fld }}"/>
																	@php $lbl='Select Subject:';
																	// echo "Test - " .@$supp_subject_arr[$k]['subject_id'];
																	
																	// $edit_class = 'subject_'.@$supp_subject_arr[$k]['subject_id'];
																	$edit_class = 'subject_'.$k;
																	
																	$placeholder = "Select Subject"; 
																	$fld='subject_id[]'; @endphp
																	<h8>@php  
																	//dd($subject_list_origional);
																	//dd($subject_list);
																	echo $lbl.Config::get('global.starMark'); @endphp </h8>
														...
														@endphp
															<tr>
																<td> 
																	<?php
																		if(!empty($subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']]) && $subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']]!=null ){
																			echo '<span style="font-size:18px;font-weight: bold;">'.$subject_list_origional[@$supp_subject_arr[$k]['origional_subject_id']].'</span>';
																		}else {
																...
																<td>
																	...
																	<span style="color:green;font-size:24px"><b>PASS</b></span>
														...
														@php }  
														if($k == 4){
															echo "<tr><td> <b>Additional Subjects :</b> </td></tr>";
														}
														...
														@php if(@$master->adm_type == 5 && @$master->course == 10){
															
														} else { @endphp
															
														@for($k=6; $remaining_subject_count > $k; $k++)
															<tr>
																<td style="font-size:18px;font-weight: bold;"> SELECT SUBJECT</td>
																<td> 
																	@php 
																	$fld='origional_subject_id[]';
																	//echo @$supp_subject_arr[$k]['origional_subject_id'];
														...
														<div class="col x25 m12 s12"> 
															<table>
																<tr> 
																	<td>@include('elements.supplementary_document_input') 
																	</td>
																</tr>
															</table>
														</div>
