											...
											<table>
												<thead>
													<tr>
														...
														<th style='text-align:center'>Is Absent</th>
														<th style='text-align:center'>Is NR</th>
																...
																<td>
																	@if(@$data->theory_absent=='1')
																		<?php echo "Absent";
																		  $minmarks=0;  
																		   ?>
																		<input type='hidden' id="theorymarks<?php echo $i; ?>" name='data[{{ $k }}][final_theory_marks]' class="final_practical_marks final_practical_marks_<?php echo $i; ?>  check_absent_marks"  disabled>
																		<input type='hidden' name='data[{{ $k }}][student_allotment_marks_id]' class='student_allotment_marks_id' value='{{Crypt::encrypt($data->id)}}' disabled>
																	...
																	<input type='hidden' id="theorymarks<?php echo $i; ?>" name='data[{{ $k }}][final_theory_marks]' class="final_practical_marks final_practical_marks_<?php echo $i; ?>  check_absent_marks" disabled>
																		<input type='hidden' name='data[{{ $k }}][student_allotment_marks_id]' class='student_allotment_marks_id' value='{{Crypt::encrypt($data->id)}}' disabled>
																	@else
																	<input type='text' id="theorymarks<?php echo $i; ?>" name='data[{{ $k }}][final_theory_marks]' class='num' class="final_practical_marks final_practical_marks_<?php echo $i; ?>  check_absent_marks" >
												...
												<div class="row">
												 ...
												 <input type='hidden' name='min_marks' class="min_marks" value='0'>
													...
													<?php if($result->lastPage()==$result->currentPage()){ ?>
														...
														<button class="btn cyan waves-effect waves-light right" type="submit" name="action">preview
														<i class="material-icons right">send</i>
														</button>
													</div>
													<?php } else { ?>
														<div class="col m3 s12 mb-3" style="margin-top:1%">
														<button class="btn cyan waves-effect waves-light right" type="submit" name="action">Submit & Next
														<i class="material-icons right">send</i>
														</button>
													</div>
													<?php } ?>
