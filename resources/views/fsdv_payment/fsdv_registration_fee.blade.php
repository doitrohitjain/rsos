							...
							<div id="Form-advance" class="card card card-default scrollspy">
								...
								<div class="card-content">
										...
										if(@$student->challan_tid && $student->challan_tid != ""){ ?> 
											<div class="">
												<p style="color:green;font-size:22px;">
													...
													(After payment, you download the clarification application's form pdf format.)
												</p>
												...
												style ="" 
												href="{{ route('fsdv_generate_student_pdf',Crypt::encrypt(@$student->student_id)) }}">
													Download Clarification Payment PDF
												</a> 
												<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange" 
												style ="" 
												href="{{ route('landing') }}">
									...
									<div class="row">
										...
										<div class="input-field col s8" style="font-size:16px;">
											@php 
												$lbl = __('Grand Total of Clarificaton Form filling <b><u> Fee Payment Amount</b></u>'); $lblhi = __('(स्पष्टीकरण आवेदन पत्र भरने का शुल्क <b><u>भुगतान राशि का कुल योग</b></u>)');
												$fld='application_fee'; 
											@endphp 
								...
								<!-- Fee payment button start -->
									<div class="card-content">
										...
										@php $isAllowPayment = CustomHelper::helpercheckIsClarificationMakePaymentAllowOrNot(@$user->id);@endphp
										@if( @$isAllowPayment && $isAllowPayment == true)
											@if( $student->challan_tid == "")
												<div class="card-content invoice-print-area">
													<div class="row">
														<div class="col m12 s12">
															@php $lbl='Make Payment'; $fld='makepayment'; @endphp 
															@php $lbl1='Declaration'; $fld1='Declaration'; @endphp 
															{{ Form::open(['route' => [request()->route()->getAction()['as'],$eenrollment], 'id' =>$model]) }} 
															{!! Form::token() !!}
															{!! method_field('PUT') !!} 
										...
										@else
											...
											<span style="font-size:18px;color:red;">
													स्पष्टीकरण हेतु भुगतान जमा करने की तिथि समाप्त हो गई है।(Submission of payment date for clarification has ended.)
											</span>
											<hr/>
										@endif
 
										<div class="card-content">
											<?php if($isAlreadyRaisedRequest > 0 && $student->challan_tid == null ){  ?>
											<br/>
											<hr/>
											<img class="" src="{{asset('public/app-assets/images/new.jpg')}}" height="40" alt="materialize logo"/>
											...
											<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange" 
											style ="" href="{{ route('fsdv_verify_request',Crypt::encrypt(@$student->enrollment)) }}">
												Verify Your Payment
											</a> 
											...
											if( $issueCount <= 0){  ?>
												...
												In case you want to raise a request to admin for your transaction got failed, please click on the raise a request button.
													...
													<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange"  href="{{ route('fsdv_raise_request',Crypt::encrypt(@$student->enrollment)) }}">
										...
										<div class="card-content">
											...
											if(@$student->challan_tid && $student->challan_tid != ""){ ?>
												<?php 
													if(@$student->reval_students->locksumbitted && @$student->locksubmitted_date){ ?>
														<div class="">
															<p style="color:green;font-size:22px;">
																पेमेंट करने के बाद आप एप्लीकेशन फॉर्म पीडीफ़ प्रारूप में डाउनलोड कर लें। 
															...
															<a class="btn cyan waves-effect waves-light border-round gradient-45deg-purple-deep-orange" 
															style ="" 
															href="{{ route('fsdv_generate_student_pdf',Crypt::encrypt(@$student->reval_students->student_id)) }}">
...
@section('customjs')
	<script src="{!! asset('public/app-assets/js/bladejs/reval_payment/reval_registration_fee.js') !!}"></script> 
@endsection