    ...
    @foreach(@$formFields as $k => $v)
        ...
        @if(@$v['input_type'] == 'text')
           ...
           @elseif(@$v['input_type'] == 'textarea')
            @php $placeholder = "Enter ". @$v['placeholder']; $fld= @$v['fld'] @endphp
             <h8>{!!Form::label($fld, $lbl) !!} @php echo $starSymbol; @endphp</h8>
            <textarea name="{{ $fld }}" class="form-control" . {{ $fld }}   autocomplete="off" id="{{ $fld }}" rows="5" cols="100" style="height: 10rem;" placeholder="Enter {{ $placeholder }}">{{ @$master[$fld] }}</textarea>
		
		  @elseif(@$v['input_type'] == 'selectrole')
		   {!!Form::label($fld, $lbl) !!} @php echo $starSymbol; @endphp</h8><br>
        ...
        @endphp 
            ...
            @if(@$v['is_multiple'] == true)
                {!! Form::select($fld. '[]',@$v['options'],@$master->$fld, ['class' => 'select2 browser-default form-control center-align ' . $fld  , 'multiple' => 'multiple','id'=>$fld, ]) !!} 
            @else
