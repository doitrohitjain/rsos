...
@if(@$masterInputArr)
	<fieldset style="display:block;">
		...
		@if(@$isCurrentSuperAdmin_id)
			<span style="" width="20%">
				<span style="color:blue">AO Status:  
				@if($lastEnteredBy->ao_status == 3)
					<span class="chip lighten-5 red red-text">
						Rejected
					</span>
				@elseif($lastEnteredBy->ao_status == 2)
					<span class="chip lighten-5 green green-text">
						Approved
					</span>
				@endif if document is correct then check the checkbox otherwise uncheck the the checkbox.</span>
			</span>
		@endif
				...
				<div class="card-content">
					<table class="responsive-table">
						...
						@foreach($v['lower_level'] as $lk => $lv)
							{{-- @dd($lv); --}}
							{{-- @dd($ao_upper_documents_verification); --}}
							{{-- @dd($lv->field_id); --}}
						<tr class="">
							@php 
