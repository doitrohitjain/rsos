		...
		@foreach($finalArr as $subject_id => $subejctData)
			...
			<table style="width:100%;"> 
				...
				<thead> 
					...
					<th>Theory Marks in Result after Reval</th>
					<th>Sessional Marks in Result</th>
					<th>Practical Marks in Result</th>
					<th>Total Marks in Result before Reval </th>
					<th>Total Marks in Result after Reval </th>
					<th>Final Result before Reval</th>
					<th>Final Result in Result after Reval</th>
				</thead>
				...
				@foreach($subejctData as $k => $v)
					...
					<tr>
						...
						<td>{{ @$v->reval_type_of_mistake }}</td> 
						<td>{{ @$v->final_theory_marks }}</td> <!-- Before -->
						<td>{{ @$v->final_theory_marks_after_reval }}</td> <!-- After -->
						...
						<td> 
							@php $rr = @$v->final_result_after_reval;$fld='final_result_after_reval'; @endphp 
							@if(isset($resultsyntax[@$v->$$fld]))
								@php $rr = $resultsyntax[@$v->$$fld]; @endphp 
							@endif
							{{ @$rr }}
						</td> 
					</tr>
