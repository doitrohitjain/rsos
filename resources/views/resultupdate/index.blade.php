					...
					<div class="card-content">
                         <h5><b>Update Subjects Marks</b></h5>
										...
										@if($data->count(0))
											@foreach($data as $data)
												...
												<tr>
													...
													@if($data->exam_month ==1 || $data->exam_month ==2)
									                <td>{{ @$exam_month_session[@$data->exam_month] }}</td>
													@else
													<td>{{ @$data->exam_month }}</td>
													@endif
													<td><b>{{ @$resultsyntax[@$data->final_result]}}<b></td>
													...
													<td>
														@if(in_array("edit_subjects_marks",$permissions))
															...
															<a href=" {{ route('updatePastStudentSubjectmarks',Crypt::encrypt($data->id)) }}" class="invoice-action-edit">
							...
							<div class="row">	
								...
								@if($data2==0)
									@if($pastdataroute==1)
									    <a href="{{ route('pastdatafinalupdate',Crypt::encrypt(@$data->enrollment)) }}"class="btn cyan waves-effect waves-light right show_confirm" style="background: linear-gradient(45deg,#303f9f,#7b1fa2);width:30%">Update Final Result</a>
									@else
