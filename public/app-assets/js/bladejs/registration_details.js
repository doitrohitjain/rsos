...
// <script type="text/javascript">
	...
	$('.stream').on("select2:select", function(event){
		...
		if (stream==1) {
			$('.adm_type').html('<option selected="selected" value="" >Select प्रवेश प्रकार (Admission Type)</option><option value="1">General Admission(सामान्य प्रवेश)</option><option value="2">Re-admission(पुनः प्रवेश)</option><option value="3">Part Admission(भाग प्रवेश)</option><option value="4">Improvement(अंक  सुधार)</option><option value="5">ITI(आईटीआई)</option>');
		} 
		if (stream==2) {
			$('.adm_type').html('<option selected="selected" value="" >Select प्रवेश प्रकार (Admission Type)</option><option value="1">General Admission(सामान्य प्रवेश)</option>');
		} 
	});

	$(document).on("click",".janAadharActionCls",function() {
	...
	$(document).on("click","#btnSearchJanAadharBtn2",function() { 
		...
		//if(jan_aadhar_nubmer.length > 0 && jan_aadhar_nubmer.length < 15){
	...
	$(document).on("focusout","#jan_aadhar_number",function() { 
		$("#member_number").val('');
		showLoading();
		var jan_aadhar_nubmer = $("#jan_aadhar_number").val();
		var length = jan_aadhar_nubmer.length;
		if(jan_aadhar_nubmer.length <= 15){ 
			...
			$.ajax({
				...
				success: function (response) { 
					...
					if(response.cmsg != 110){
						...
						var img = "<img class='' src='../public/app-assets/images/new.jpg' height='40' alt='materialize logo'/>";
						var continueBtn = '<div class=""><button class="btn cyan waves-effect waves-light right janAadharActionCls" type="button" id="janAadharAction" style="    background-color: #f51919 !important;font-size: larger;font-weight: bolder;">Continue</button></div>'; 

						html += "<table class='table'>";
						
						html += "<tr><th colspan='8'><b><center>" +  img +"<span style='color:red;font-size:20px;'>Please select any one member and press Continue button.</span></center></b>";
						html += continueBtn + ' </th><tr>';
						html += "<tr><th width='50px'>Choose</th><th width='50px'>#</th><th>Name</th><th>Father's Name</th><th>Mother's Name</th><th>DOB(DD-MM-YYYY)</th><th>Jan Member Id</th><th>AADHAR</th><tr>";
						...
						if(response_type=='[object Object]'){ // single member loop
							...
							$.each(members, function(k, v) { 
								var janId = members.jan_mid;
								if(members.jan_mid == 0){
									janId = members.hof_jan_m_id
								}
								var cls = 'inputOption' + counter;
								if(k == "nameEng"){	
									html += '<td><p class="mb-1 inputOption' + counter +'"><label><input data-jan_id="' + members.jan_mid +'" data-id="' + counter +'" class="memberchooseradioinput with-gap memberRadioOption ' + counter + '_radio_input_jan_id" value="' + counter + '" name="' + k + '" type="radio"><span></span></label></p></td>';
									html += '<td>' + (counter) + '</td>';									
								...
								if(k == "jan_mid"){
									if(v == 0){
										v = members.hof_jan_m_id;
									}
									html += "<td>" + v +"</td>"; 
									if(v != ""){
										var action = "_isJanNumberExists";
							...
							$.each(members, function(ok, ov) {   
								html += "<tr>";
								counter++;
								$.each(this, function(k, v) { //entDetails[keyCounter].jan_mid
									var cls = 'inputOption' + counter;
									var jan_mid_jq = entDetails[keyCounter].jan_mid;
									var hof_jan_m_id = ov.hof_jan_m_id;
									 
									if(jan_mid_jq == 0){
										jan_mid_jq = hof_jan_m_id;
									}
									 
									if(k == "nameEng"){	
										html += '<td><p class="mb-1 inputOption' + counter +'"><label><input data-jan_id="' + jan_mid_jq +'" data-id="' + counter +'" class="memberchooseradioinput with-gap memberRadioOption ' + counter + '_radio_input_jan_id" value="' + counter + '" name="' + k + '" type="radio"><span></span></label></p></td>';
										html += '<td>' + (counter) + '</td>';									
										html += "<td class='" + counter + "_itmename'><b>" + v +"</b></td>";
									}
									if(k == "fnameEng"){
						...
						// htmlFooter += '<button class="btn cyan waves-effect waves-light right janAadharActionCls" type="button" id="janAadharAction2">Continue';
						// htmlFooter += '</button></br></br></div>';
						$('.submitBtn').removeClass('disabled');
	...
	$(document).ready(function() {
		...
		var aicode = $("#selectedAiCenterId").html();
		aicode = aicode.replace("Contact:", '<br><span style="color:brown;">');
		$("#selectedAiCenterId").html(aicode);



		//first check with ajax server side validaiton if true then submit
		var ajaxBaseUrl = phpJsVarSet.routes.ajaxBaseUrl;
		var action = "ajaxRegistrationDetilasValidation";
		...
		$("#Student").validate({
			...
			submitHandler: function(form) {  
				if($('#ajaxRequest').val()==1){
					showLoading(); 
					$.ajax({
						url: config.routes.ajaxRegistrationDetilasValidation,
						type: "POST",
						dataType: "json",
						...
						success: function (response) { 
							...
							if(response.isValid==false){
								...
								//here ajax hit and check is present aur not
								var clsName = "api";
		...
		//formId = "#temp" + config.data.formId; 
		//$(formId).validate({
			//rules: {
