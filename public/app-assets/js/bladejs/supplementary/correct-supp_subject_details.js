	...
	$('.supp_form_submit').on("click",function(event){
		if($('.supp_form_document_field').val() == '' &&  $('#marksheet_doc_hidden').val() == '') { 
			swal({
				html:true,
				title: "मान्यता त्रुटि(Validation Error)",
		...
		var filename = $(".supp_form_document_field").val();
		var fileNameExt = filename.replace(/^.*\./, '');
		fileNameExt = fileNameExt.toLowerCase();
		if ($.inArray(fileNameExt, validExtensions) == -1 && $('.supp_form_document_field').val() != ''){  
			swal({
				title: "मान्यता त्रुटि(Validation Error)",
