// <script type="text/javascript">
	...
	$(document).on("click",".memberchooseradioinput",function() {
		var dataItem = $(this).val(); 
		var dataItemCls = "." + dataItem + "_itmename";
		var name = $(dataItemCls).val();
		$("#janaadharfeatchedname").val(name);
		return false;
	})
			...
			$.ajax({
				...
				success: function (response) { 
					...
					if(response.cmsg != 110){
						...
						var html = "<table class='table' id='memberchoose'>";
						html += "<tr><th colspan='8'><b><center>Choose one member</center></b></th><tr>";
						...
						if(response_type=='[object Object]'){ // single member loop
							...
							$.each(members, function(k, v) { 
								...
								if(k == "nameEng"){	
									html += '<td><p class="mb-1 inputOption' + counter +'"><label><input data-id="' + counter +'" class="memberchooseradioinput with-gap" value="' + counter + '" name="' + k + '" type="radio"><span></span></label></p></td>';
									html += '<td>' + (counter) + '</td>';									
									html += "<td><b>" + v +"</b></td>";
								}
								if(k == "fnameEng"){
									html += "<td class='" + counter + "_itmename'>" + v +"</td>";
								}
								if(k == "mnameEng"){
									html += "<td>" + v +"</td>";
								}
							...
							$.each(members, function() { 
								...
								$.each(this, function(k, v) { 
									var cls = 'inputOption' + counter;
									if(k == "nameEng"){	
										html += '<td><p class="mb-1 inputOption' + counter +'"><label><input class="memberchooseradioinput with-gap" value="' + counter + '" name="' + k + '" type="radio"><span></span></label></p></td>';
										html += '<td>' + (counter) + '</td>';									
										html += "<td><b>" + v +"</b></td>";
									}
									if(k == "fnameEng"){
