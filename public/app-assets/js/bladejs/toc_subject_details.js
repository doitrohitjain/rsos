	...
	$('.toc_subject_dropdown').on("select2:select", function(event){
		...
		if(subject_id != ''){
			...
			$.ajax({
				...
				data: {'subject_id': subject_id,'board_id':board},
				success: function (data) { 
					$('#theory_'+ subjectsrno).val('');
					$('#practical_'+ subjectsrno).val('');
					$('#total_'+ subjectsrno).val('');
					if(data==false){
	...
	$('.toc_roll_no').on("change", function(event){
		...
		if(toc_roll_no!='' && board==81 && course!=''){
			showLoading();
			...
			$.ajax({
				...
				data: {'toc_roll_no': toc_roll_no,'course': course},
