  // jQuery Form Validator
	 $(document).ready(function() {
		...
		var formId = "#revisedresult";
		var action = "checkreviedresultstudent";
		...
		$(formId).validate({
			...
			submitHandler: function(form) { 
				...
				if($('#ajaxRequest').val()==1){
					showLoading(); 
					$.ajax({
						url: config.routes.checkreviedresultstudent,
						type: "POST",
						...
						if(data == false){
							swal({
								title: 'Revised Result not found as per given details.',
								icon: 'error',
								button: "Close",
							});
							...
							swal({
								title: 'Are you sure you want to see revised result.',
								text: "Please confirm",
								icon: 'success',
								buttons: true,
							})
