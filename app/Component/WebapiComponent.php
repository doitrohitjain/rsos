...
class WebapiComponent {
    ...
    public function getAPIStudentLoginDetails($ssoid=null,$dob=null,$enrollment=null){
        $master = array();
        $studentFields = array('students.id','students.course','students.enrollment','students.name','students.dob','students.ssoid','students.student_status_at_different_level','students.exam_year','students.exam_month');
        ...
        if (Cache::has($cacheName)) {
            ...
            $student = Cache::rememberForever($cacheName, function () use ($ssoid,$dob,$enrollment,$studentFields,$admissistuon_academicyear_id,$current_exam_month_id) {
				 
				if(@$enrollment){
					return $result = $student = Student::where('students.ssoid',$ssoid)->where('students.enrollment',$enrollment)->where('students.dob',$dob)
                ...
                return $result = $student = Student::Join('applications', 'applications.student_id', '=', 'students.id')->where('applications.locksumbitted','1')
				->where('students.ssoid',$ssoid)->where('students.dob',$dob)
				->where('students.exam_year',$admissistuon_academicyear_id)->where('students.exam_month',$current_exam_month_id)
				->orderBy('students.exam_year','DESC')->orderBy('students.exam_month','asc')
				->first($studentFields); 
            });
        }   
	...
	public function getAPIStudentEnrollmentList($ssoid=null,$dob=null){
		...
		if (Cache::has($cacheName)) {
			...
			$student = Cache::rememberForever($cacheName, function () use ($ssoid,$dob,$studentFields,$admissistuon_academicyear_id,$current_exam_month_id) {
				...
				return $result = $student = Student::where('students.ssoid',$ssoid)->where('students.dob',$dob)
				// ->where('students.exam_year',$admissistuon_academicyear_id)
				->whereNotNull('students.enrollment')
				// ->where('students.exam_month',$current_exam_month_id)
				->orderBy('students.exam_year','DESC')->orderBy('students.exam_month','asc')
				->pluck('students.enrollment','students.id');
			});
		}  
		if(@$student){
	...
	public function getAPIStudentDetails($ssoid=null,$dob=null,$enrollment=null){
        $master = array();
        ...
        $student = Student::where('students.ssoid',$ssoid)->where('students.enrollment',$enrollment)->where('students.dob',$dob)->orderBy('students.exam_year','DESC')->orderBy('students.exam_month','asc')->first($studentFields);
