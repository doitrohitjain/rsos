...
class SessionalController extends Controller
{
    ...
    public function __construct(request $request)
    {
        ...
        parent::__construct();
        $this->middleware('permission:sessional_find_enrollment', ['only' => ['find_enrollment']]);
        $this->middleware('permission:sessional_marks_details', ['only' => ['marks_details']]);
        $this->middleware('permission:sessional_marks_preview_details', ['only' => ['marks_preview_details']]);
        $this->customComponent = new CustomComponent();
    }

    public function find_enrollment(Request $request)
    {
        return redirect()->route('prepare_find_enrollment')->with('error', 'Something is wrong.');
        ...
        if (count($request->all()) > 0) {
            ...
            if ($master) {
                return redirect()->route('marks_details', Crypt::encrypt($student_id))->with('message', 'Enrollment details found.');
            } else {
                return redirect()->back()->with('error', 'Failed! Student not found.')->withInput($request->all());
            }
        }
        return view('sessional.find_enrollment', compact('model', 'master', 'estudent_id', 'student_id', 'page_title'));
    }
    ...
    public function marks_details(Request $request, $student_id)
    {
        ...
        if (empty($student_id)) {
            return redirect()->route('find_enrollment')->with('error', 'Please first find student enrollment number.');
        }
        ...
        $examSubjectsList = $this->_getExamSubjectsList($student_id);
        if (count($examSubjectsList) <= 0) {
            return redirect()->route('find_enrollment')->with('error', 'Failed! Student exam subjects not found.')->withInput($request->all());
        }
        if (count($request->all()) > 0) {
            $modelObj = new ExamSubject;
            $isValid = true;
            $customerrors = null;
            $inputs = $request->all();
            ...
            if ($isValid) {
                ...
                foreach ($request->subject_id as $key => $value) {
                    ...
                    $studentupdatesessionalmarks = ExamSubject::where('exam_month', $exam_month)->where('exam_year', $exam_year)->where('student_id', $student_id)->where('subject_id', $key)->update($studentsessionalmarks);
                }
                return redirect()->route('marks_preview_details', Crypt::encrypt($student_id))->with('message', '' . 'Marks has been saved.');
            } else {
                return redirect()->back()->withErrors($customerrors)->withInput($request->all());
            }
        }
        return view('sessional.marks_details', compact('maxMarks', 'minMarks', 'subject_list', 'master', 'examSubjectsList', 'model', 'estudent_id', 'student_id', 'page_title'));
    }
    ...
    public function marks_preview_details(Request $request, $student_id)
    {
        ...
        $studentdata = Student::findOrFail($student_id);
        $master = $this->getStudentPersoanlDetails($student_id);
        $subject_list = $this->subjectList($studentdata->course);
        $examSubjectsList = $this->_getExamSubjectsList($student_id);
        if (empty($master)) {
        ...
        return view('sessional.marks_preview_details', compact('model', 'masterrecord', 'master', 'examSubjectsList', 'estudent_id', 'student_id', 'documentErrors', 'page_title'));
    }
