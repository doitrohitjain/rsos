...
use App\Exports\AiCenterCourseWiseStudentCountExlExport;
...
use App\Exports\CourseMediumWiseBookSubjectCountExlExport;
use App\Exports\DownloaddistrictwisecategoryExlExport;
use App\Exports\Examcenterwiseexcel;
use App\Exports\PrepareSessionalExlExport;
use App\Exports\SessionalExlExport;
use App\Exports\Studentfeeorgsummaryexcel;
use App\Exports\StudentFeesExlExport;
use App\Exports\StudentFeesOrgExlExport;
...
class ExaminationReportController extends Controller
{
    ...
    function __construct()
    {
        ...
        //$this->middleware('permission:prepare_examination_report_sessional_report_h', ['only' => ['prepare_sessional_report_h']]);
    }
    ...
    public function examcenter_material_zip_download(Request $request, $course = null)
    {
        ...
        $current_admission_year_string = Config::get("global.current_materials_exam_year");
        $exam_month = Config::get('global.current_materials_exam_month');
        ...
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($current_admission_year_string);
        $examcentermaterial = "examcentermaterial";
        $examcentermaterial_zip = "examcentermaterial_zip";
        $nextPath = $current_folder_year . "/" . $examcentermaterial . "/" . $exam_month . "/" . $course . "/" . $districtNameListname . "/" . $master->$centerCode . "/";
    ...
    public function downloadStudentorgFeesExl(Request $request, $type = "xlsx")
    {
        $application_exl_data = new StudentFeesOrgExlExport;
        $filename = 'application_data' . date('d-m-Y H:i:s') . '.' . $type;
        return Excel::download($application_exl_data, $filename);
    }
    ...
    public function sessional_report_h(Request $request)
    {
        ...
        $custom_component_obj = new CustomComponent;
        $aiCenters = $custom_component_obj->getAiCenters();
        $alsoNotAList = collect(["a" => "a", "b" => "b", "c" => "c"]);
        ...
        $isAdminStatus = $custom_component_obj->_checkIsAdminRole();
        $condtions = array();
        $stream_id = Config::get('global.current_sessional_exam_month');
        $condtions["students.stream"] = $stream_id;
        $condtions["students.exam_year"] = CustomHelper::_get_selected_sessions();
        if ($isAdminStatus == false) {
            $role_id = @Session::get('role_id');
    ...
    public function prepare_sessional_report_h(Request $request)
    {
        ...
        $exportBtn = array(
            array(
                "label" => "Export Excel",
                'url' => 'downloadPreparesessionalexportExl',
                'status' => true,
            ),
            array(
                "label" => "Export PDF",
        ...
        $master = $custom_component_obj->getPrepareSessionalDataH($formId);

        return view('examination_reports.prepare_sessional_report',
            compact('tableData', 'master', 'aiCenters', 'exportBtn', 'formId', 'table_id', 'filters',
                'title', 'breadcrumbs', 'gender_id', 'yes_no', 'yes_no_temp', 'midium'))->withInput($request->all());
    }
    ...
    public function downloadPreparesessionalexportExl()
    {
        $examcenter_exl_data = (new PrepareSessionalExlExport);
        return Excel::download($examcenter_exl_data, 'Sessional.xlsx');
    }
    ...
    public function studentchecklists(Request $request)
    {
        ...
        $combo_name = 'stream_id';
        $stream_id = $this->master_details($combo_name);
        $latefees = array("0" => "0", "250" => "250", "500" => "500");

        $exportBtnArr = array();
        $stream = 1;
        $i = 0;
        $exportBtnArr[$i]['course'] = 10;
        $exportBtnArr[$i]['stream'] = $stream;
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = null;
        $exportBtnArr[$i]['title'] = "Course 10th etc.";
        $i++;
        $exportBtnArr[$i]['course'] = 10;
        $exportBtnArr[$i]['stream'] = $stream;
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = 0;
        $i++;
        $exportBtnArr[$i]['course'] = 10;
        $exportBtnArr[$i]['stream'] = $stream;
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = 250;
        $i++;
        $exportBtnArr[$i]['course'] = 10;
        $exportBtnArr[$i]['stream'] = $stream;
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = 500;
        $i++;
        $exportBtnArr[$i]['course'] = 12;
        $exportBtnArr[$i]['stream'] = $stream;
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = null;
        $i++;
        ...
        $exportBtnArr[$i]['ai_code'] = 0;
        $exportBtnArr[$i]['latefee'] = 500;
        return view('examination_reports.student_check_lists_report', compact('exportBtnArr', 'aiCenters', 'exportBtn', 'title', 'breadcrumbs', 'courses', 'stream_id', 'latefees'));
    }
    ...
    public function olddownloadstudentchecklistsPdf($course = null, $stream = null, $ai_code = null, Request $request)
    {
        ...
        Storage::put("public/files/" . $current_folder_year . "/" . "checklist" . "/" . $stream . "/" . $course . "/" . $ai_code . "/" . $filename, $pdf->output());
        ...
        $path = public_path("files/" . $current_folder_year . "/" . "checklist" . "/" . $stream . "/" . $course . "/" . $aicode . "/");
    ...
    public function downloadTocCheckListsPdf($course = null, $stream = null, $ai_code = null, Request $request)
    {
        ...
        $exam_year = Config::get("global.current_checklist_exam_year");
        $title = "Toc checklist Report";
        ...
        $combo_name = 'minage';
        $minage = $this->master_details($combo_name);
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($exam_year);
        //$rsos_years = $this->getListRsosYears();
        $combo_name = "year";
        $rsos_years = $this->master_details($combo_name);
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            $master = Student::where($conditions)->whereNull('students.student_status_at_different_level')
            ...
            $path = public_path("files/reports/" . $current_folder_year . "/" . $checklist . "/stream" . $stream . "/" . $course . "/");
        ...
        if (@$ai_code) {
		 ...
		 $folder_path = "files/reports/" . $current_folder_year."/". $checklist ."/". "stream" . $stream ."/". $course;
    ...
    public function getDownloadtocchecklistsingleaicode(Request $request)
    {
        ...
        elseif ($request->type == 33) {
            ...
            if ($request->late_fee !== null && $request->late_fee >= 0) {
                $filename = $request->late_fee . '_' . $request->ai_code . '_checklist.pdf';
            } else {
                $filename = $request->ai_code . '_checklist.pdf';
            }
        }
        $path = public_path("files/reports/" . $current_folder_year . "/" . $checklistall . "/" . "stream" . $request->stream . "/" . $request->course . "/" . $filename);
    ...
    public function getDownloadfixcodesingle(Request $request)
    {
        $request->validate([
            'course' => 'required',
            'stream' => 'required',
        ...
        $exam_year = Config::get("global.current_materials_exam_year");
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($exam_year);
        $fixcode = "fixcode";
        $filename = 'student_fixcode_code_' . $request->ecenter . '.pdf';
        $path = public_path("files/reports/" . $current_folder_year . "/" . $fixcode . "/" . $request->stream . "/" . $request->course . "/" . $request->subjects . "/" . $filename);

        if (File::exists($path)) {
            return Response::download($path);
        } else {
            return redirect()->route('enrollment_fixcode_views')->with('error', 'Fixcode Not Generated');
        }
    }
    ...
    {
        $current_admission_year_string = Config::get('global.current_checklist_exam_year');
        $exam_month = Config::get('global.current_checklist_exam_month');
        ini_set('memory_limit', '3000M');
        ini_set('max_execution_time', '0');
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($current_admission_year_string);
        $streamname = 'stream';
        ...
        $nextPath = $current_folder_year . "/" . $checklistall . "/" . $streamname . $stream . "/" . $course . "";
        $folder_path = public_path("files/reports/" . $nextPath);
    ...
    {
        $stream = Config::get("global.current_materials_exam_month");
        if (!empty($request->stream)) {
            $stream = $request->stream;
        }
        if (!empty($request->pagecount)) {
        ...
        $combo_name = 'adm_type';
        $adm_types = $this->master_details($combo_name);
        $defaultOption = array("0" => "Number OF Record", "20" => "20", "60" => "60", "80" => "80", "100" => "100", "150" => "150", "200" => "200");
        $defaultOptions = collect($defaultOption);
        ...
        $conditions["center_allotments.exam_year"] = Config::get("global.current_materials_exam_year");
        $conditions["pagecount"] = $pagecount;
        Session::put($formId . '_condtions', $conditions);
        $master = $custom_component_obj->getExamcenterAllotmentDataForReport($formId);
        return view('examination_reports.reportexamcenterwise', compact('pagecount', 'defaultOptions', 'stream', 'master', 'exportBtn', 'title', 'breadcrumbs', 'filters'))->withInput($request->all());
    ...
    public function reportexamcenterwisePdf()
    {
        ...
        $path = public_path("files/reports/" . $current_folder_year . "/" . "examcenterwise" . "/" . $stream);
        File::makeDirectory($path, $mode = 0777, true, true);
        $pdf->setOption('footer-right', 'Page [page] of [toPage]');
    ...
    public function reportexamcenterwiseview()
    {
    ...
    public function Nominalnrgenerateview(Request $request)
    {
        ...
        $current_admission_session_id = Config::get("global.current_materials_exam_year");
        $current_exam_month_id = Config::get("global.current_materials_exam_month");
        $course = 10;
        $coursess = 12;
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($current_admission_session_id);
        ...
        $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $District->name . "/" . $ai_codes . "/" . $filename);


        $path2 = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $coursess . "/" . $District->name . "/" . $ai_codes . "/" . $filename);
    ...
    public function getDownloadnominalrollaicodecenterwise($course = null, $stream = null, Request $request)
    {
        ...
        $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $District->name . "/" . $ai_code . "/" . $filename);
        if ($user_role == $developeradminrole || $user_role == $examination_department) {
            return $path;
        } else {
            return Response::download($path);
    ...
    public function Nominalnrpdf($course = null, $stream = null, $ai_code = null, Request $request)
    {
        ...
        if (@$ai_code) {
            ...
            $aiCenters = $custom_component_obj->getAiCentersformaterial();
        }
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            })->with('Supplementarysubjects', function ($query) use ($exam_year, $stream) {
                $query->whereNull('deleted_at')->where('exam_year', $exam_year)->where('exam_month', $stream);
            })
            ...
            if (@$ai_code) {
                ...
                $pdf->setOption('footer-right', 'Page [page] of [toPage]');
                $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $aicodedistrictid1 . "/" . $aicodetemp . "/");
                File::makeDirectory($path, $mode = 0777, true, true);
    ...
    public function board_nr_student_enrollment_view()
    {
        ...
        $combo_name = 'stream_id';
        $stream_id = $this->master_details($combo_name);
        return view('examination_reports.boardnrstudentenrollmentexcel', compact('breadcrumbs', 'title', 'courses', 'stream_id'));
    }
    ...
    public function getStudentFeeSummaryPay(Request $request)
    {
        ...
        // 				if( !empty($iv['dbtbl']) && isset($inputs[$iv['fld']]) && !empty($inputs[$iv['fld']]) ){
        // 					if($iv['fld']=='late_fee' && $inputs[$iv['fld']] == 1){

        // 						$condtions[$iv['dbtbl'] . "." . $iv['fld']] = 0;
        // 					}elseif($iv['fld']=='late_fee' ){
        // 						$inputs[$iv['fld']];
                ...
                if ($k != 'page' && $v != "") {
                    foreach ($filters as $ik => $iv) {
                        if (!empty($iv['dbtbl']) && $iv['fld'] == $k) {
                            if ($iv['fld'] == 'late_fee' && $inputs[$iv['fld']] == 1) {
                                $symbol = ">";
                            } elseif ($iv['fld'] == 'late_fee' && $inputs[$iv['fld']] = 0) {
                                $symbol = "<=";
                            } else {
                                $condtions[$iv['dbtbl'] . "." . $k] = $v;
                            }
    ...
    public function mobile_view_hallticketbulview($enrollment = null)
    {
        $enrollment = Crypt::decrypt($enrollment);
        $studentFields = array('students.id', 'students.enrollment', 'students.course', 'students.stream', 'students.ai_code');
        ...
        echo "Hall ticket of " . ($enrollment);
        die;
        if (@$student) {
            $master['student'] = $student->toArray();

            $student_id = $master['student']['id'];
            $course = $master['student']['course'];
            $stream = $master['student']['stream'];
            $ai_code = $master['student']['ai_code'];
            ...
            if (@$isStudent) {
                ...
                $enrollment = Auth::guard('student')->user()->enrollment;
                $ai_code = Auth::guard('student')->user()->ai_code;
                $exam_monthyear = Config::get('global.current_materials_exam_year');
                $stream = Config::get('global.current_materials_exam_month');
            } else {
                $exam_monthyear = Config::get('global.current_materials_exam_year');
            }

            $finedstudentallotment = StudentAllotment::where('ai_code', $ai_code)->where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->first('course');
    ...
    public function downloadhallticketbulviewpdf(Request $request)
    {
        ...
        //       'course' => 'required',
        //   ]);
        return redirect()->route('hall_ticket_single_enrollment_mobile_view', array($request));
    }
    ...
    public function hall_ticket_bulk_download($course = null, $stream = null, $ai_code = null, Request $request)
    {
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            /* Suplementary Data Start  */
            $conditionSupplementary [] = ['student_allotments.ai_code', '=', $ai_code];
            $conditionSupplementary [] = ['student_allotments.exam_year', '=', $exam_year];
            $conditionSupplementary [] = ['student_allotments.supplementary', '=', 1];
            $conditionSupplementary [] = ['student_allotments.exam_month', '=', $stream];
            $conditionSupplementary [] = ['supplementaries.is_eligible', '=', 1];
            $conditionSupplementary [] = ['supplementaries.exam_year', '=', $exam_year];
            $conditionSupplementary [] = ['supplementaries.exam_month', '=', $stream];
            ...
            if (isset($students) && !empty($students)) {
                foreach ($students as $stKey => $student) {
                    ...
                    if (isset($student->course) && !empty($student->course)) {
                        ...
                        $dataSave[$student->course][$key]['course'] = null;
                    }
    ...
    public function api_hall_ticket_for_mobile_single_enrollment_viewold(Request $request)
    {
        $ai_code = null;
        $stream = null;
        $exam_monthyear = null;
        ...
        //$student_id = "33333";$ssoid = "00DHARASINGH.DHOBI";$dob = "1999-05-18";
        $ssoid = @$inputs['ssoid'];
        ...
        /* Secure Token check end */
        //eyJpdiI6IjUxeVpKOVk1RXJPdlJIZ2ladVU0b2c9PSIsInZhbHVlIjoiTFpNQzY1eGpBSDJFekg1YzAyTFJTUT09IiwibWFjIjoiNmZiOTAyYjM5MmQ1YmNiZWZhNmNkYjkyZmFkZjA2MGVlNTljYzk5ZTBlZTU3OWQ3MzhiOTJmZGFhNGZkZmNmOSIsInRhZyI6IiJ9

        // $student_id = "423785";
        // $student_id = Crypt::encrypt($student_id);
        // dd($student_id);
        $custom_component_obj = new CustomComponent;
        ...
        $finedstudentallotment = StudentAllotment::where('ai_code', $ai_code)->where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->first(['course', 'id']);
        //here true for hold the hall ticket for api
        if (true) {
            echo "Coming Soon! The hall ticket for the student will be displayed here.";
            die;
        }
        if (empty($finedstudentallotment)) {
            $data = null;
            $status = false;
            $error = 'The Enrollment not found.';
            $response = array('status' => $status, "data" => $data, "error" => $error);
            print_r($response);
            die;
        }

        $isAllowToShowAdmitCardDownloadForStudent = $this->getIsAllowToShowAdmitCardDownloadForStudent(@$finedstudentallotment);

        if (!@$isAllowToShowAdmitCardDownloadForStudent) {
            $data = null;
            $status = false;
            $error = 'The download date for the admit card has ended.';
            $response = array('status' => $status, "data" => $data, "error" => $error);
            print_r($response);
            die;
        }
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            if (isset($ai_code) && !empty($ai_code)) {
                ...
                $data = null;
                $status = false;
                $error = 'Oop"s! Did you really think you are allowed to see that?';
                $response = array('status' => $status, "data" => $data, "error" => $error);
                print_r($response);
                die;
            }
            ...
            $students = StudentAllotment::select(
                ...
                ->join('students', 'students.id', '=', 'student_allotments.student_id')
                ->join('examcenter_details', 'examcenter_details.id', '=', 'student_allotments.examcenter_detail_id')
                ->where($conditions)->whereNull('student_allotments.deleted_at')->whereNull('students.deleted_at')->whereNull('documents.deleted_at')->whereNull('addresses.deleted_at')
                ->whereNull('applications.deleted_at')->whereNull('examcenter_details.deleted_at')
                ->groupBy('student_allotments.student_id')
                ->orderBy('student_allotments.student_id', 'ASC')->get();
            ...
            if (isset($suppStudents) && !empty($suppStudents)) {
                foreach ($suppStudents as $suppKey => $suppStudent) {
                    ...
                    $dataSave[$suppStudent->course][$key]['exam_subjects'] = $this->getSubjectDetailForHallTicketSupp($suppStudent->student_id, 'api');
            ...
            if (isset($students) && !empty($students)) {
                foreach ($students as $stKey => $student) {
                    ...
                    $dataSave[$student->course][$key]['exam_subjects'] = $this->getSubjectDetailForHallTicket($student->student_id, 'api');
            ...
            // $pdf =  PDF::loadView('examination_reports.hall_ticket_view',compact('current_year','stream','subjects','subjects10',
            // 'studentDocumentPath','subjects12','practicalsubjects12','subject_list','categorya','aicode','students','subreportname','reportname','course','exam_session','exam_time_table_start_end_time'));
            // $pdf->setTimeout(60000);
            // $pdf->setOption('footer-right', 'Page [page] of [toPage]');
    ...
    public function hall_ticket_single_enrollment_download(Request $request)
    {
        ...
        $stream = Config::get('global.current_materials_exam_month');
        $aicentermaterial = "aicentermaterial";
        $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
        $filename = 'hallticket_' . $enrollment . '.pdf';
        @$completepath = $path . $filename;
        if (File::exists($completepath)) {
            return Response::download($completepath);
        }
        ...
        $examcenterid = Config::get("global.examination_department");
        if ($role_id == $aicenter_id) {
            $finedstudentallotment = StudentAllotment::where('ai_code', $ai_code)->where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->first('course');
        } elseif ($role_id == $examcenterid) {
            $finedstudentallotment = StudentAllotment::where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->first('course');
        }
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            $pdf->setTimeout(2000);
            $enrollmentname = 'enrollment';
            $path = public_path("files/reports/" . $current_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
            File::makeDirectory($path, $mode = 0777, true, true);
            $pdf->setOption('footer-right', 'Page [page] of [toPage]');
            $filename = 'hallticket_' . $enrollment . '.pdf';
            $completepath = $path . $filename;
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            if (isset($suppStudents) && !empty($suppStudents)) {
                foreach ($suppStudents as $suppKey => $suppStudent) {
                    ...
                    $ai_code = @$suppStudent->ai_code;
                    $dataSave[$suppStudent->course][$key]['type'] = 'Supplementary';
                    $dataSave[$suppStudent->course][$key]['halltickectqrcode'] = $this->hallticketqrcode($suppStudent->enrollment);
                    $dataSave[$suppStudent->course][$key]['id'] = $suppStudent->id;
                    $dataSave[$suppStudent->course][$key]['student_id'] = $suppStudent->student_id;
            ...
            if (isset($students) && !empty($students)) {
                foreach ($students as $stKey => $student) {
                    ...
                    $dataSave[$student->course][$key]['index'] = $stKey;
                    $dataSave[$student->course][$key]['id'] = $student->id;
                    $dataSave[$student->course][$key]['halltickectqrcode'] = $this->hallticketqrcode($student->enrollment);
                    $dataSave[$student->course][$key]['student_id'] = $student->student_id;
                    $dataSave[$student->course][$key]['ai_code'] = $student->ai_code;
                    $dataSave[$student->course][$key]['enrollment'] = $student->enrollment;
                    $dataSave[$student->course][$key]['name'] = $student->name;
    ...
    public function downloadstudentchecklistsPdf1(Request $request)
    {
        ...
        return redirect()->route('downloadstudentchecklistsPdf', $data);
    }

    public function downloadstudentchecklistsPdf(Request $request)
    {

        ini_set('memory_limit', '10000M');
        ini_set('max_execution_time', '0');
        $aicode = $request->ai_code;
        $ai_code = $request->ai_code;
        $stream = $request->stream;
        $course = $request->course;
        $latefee = $request->late_fee;
        $exam_year = Config::get("global.current_checklist_exam_year");
        $custom_component_obj = new CustomComponent;
        if (@$ai_code) {
            $aiCenters = $custom_component_obj->getAiCenters($ai_code);
        ...
        foreach ($aiCenters as $key => $value) {
            ...
            $conditions["students.exam_year"] = $exam_year;
            @$reportname = $aicodetemp;
            //$studentIdsConditions["students.id"] = $this->getNotValidAfterDatePaymentRecivedStudentIds();
            if ($latefee !== null && $latefee >= 0) {
                $master = Student::with('application', 'document', 'address', 'toc', 'studentfees', 'admission_subject', 'toc_subject', 'exam_subject')->whereHas('studentfees', function ($query) use ($latefee) {
                    $query->where('late_fee', $latefee);
                })->where($conditions)->whereNull('students.student_status_at_different_level')
                    // ->whereNotIn($studentIdsConditions)
                ...
                $master = Student::with('application', 'document', 'address', 'toc', 'studentfees', 'admission_subject', 'toc_subject', 'exam_subject')->where($conditions)->whereNull('students.student_status_at_different_level')
                    // ->whereNotIn($studentIdsConditions)
                    ->orderBy('enrollment', 'ASC')
                    ->get(['students.id', 'students.submitted', 'students.name', 'students.father_name', 'students.mother_name', 'students.gender_id', 'students.dob',
                        'students.enrollment', 'students.adm_type', 'students.stream', 'students.course', 'students.ai_code']);
            ...
            $path = public_path("files/reports/" . $current_folder_year . "/" . $checklist . "/stream" . $stream . "/" . $course . "/");
            File::makeDirectory($path, $mode = 0777, true, true);
            if (@$aicode && $latefee !== null && $latefee >= 0) {
                $filename = $latefee . "_" . $aicode . "_checklist.pdf";
            } elseif (@$aicode && empty($latefee)) {
                $filename = $aicode . "_checklist.pdf";
            }
            ...
            foreach ($centercodeandids as $centerid => $centervalue) {
                ...
                foreach ($aiCenters as $aicode => $value) {
                    ...
                    $supp_master = StudentAllotment::with('student.application', 'examcenter')->with('Supplementary', function ($query) use ($exam_year, $stream) {
                        ...
                        $query->orderBy('student_id', 'DESC')->where('exam_year', $exam_year)->where('exam_month', $stream)->whereNull('deleted_at');
                ...
                $nextPath = $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $districtNameListname . "/" . $centervalue . "/";
            ...
            foreach ($centercodeandids as $centerid => $centervalue) {
                ...
                foreach ($aiCenters as $aicode => $value) {
                    ...
                    $supp_master = StudentAllotment::with('student.application', 'examcenter')->with('Supplementary', function ($query) use ($exam_year, $stream) {
                        ...
                        $query->orderBy('student_id', 'DESC')->whereNull('deleted_at')->where('exam_year', $exam_year)->where('exam_month', $stream);
                    })->where($supp_conditions)->whereNull('deleted_at')->orderBy('enrollment', 'ASC')->get()->toArray();
    ...
    public function oldexam_center_nominal_roll_pdf($course = null, $stream = null, $districtid = null)
    {
        ...
        foreach ($districtNameList as $districtid => $districtvalue) {
			...
			*/
                ...
                $nextPath = $current_folder_year . "/" . $aicentermaterial . "/stream" . $stream . "/" . $course . "/" . $districtNameListname . "/" . $centervalue . "/";
                $path = public_path("files/reports/" . $nextPath);
    ...
    public function excenterattendancesheet($course = null, $stream = null, $districtid = null)
    {
        ...
        foreach ($districtNameList as $districtid => $districtvalue) {
            ...
            foreach ($centercodeandid as $centerid => $centervalue) {
                ...
                $examyersexamcenter = Config::get("global.current_materials_exam_year");;
    ...
    public function enrollment_fixcode_view_bulk($subjectscode = null, $stream = null, $course = null, $start = null)
    {
        ...
        foreach ($subjectList as $subjectid => $subjectname) {
            ...
            foreach ($centercodeandids as $centerid => $centervalue) {
                ...
                $nextPath = $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $subjectname . "/";
                $path = public_path("files/reports/" . $nextPath);
    ...
    public function enrollment_fixcode_view($subjectscode = null, $stream = null, $course = null, $start = null)
    {
        ...
        foreach ($centercodeandids as $centerid => $centervalue) {
            ...
            $nextPath = $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $course . "/" . $subjectscode . "/";
            $path = public_path("files/reports/" . $nextPath);
    ...
    public function downloadsupplementaryCheckListsPdf($course = null, $stream = null, $ai_code = null, Request $request)
    {
        ...
        $current_admission_session_id = Config::get("global.current_checklist_exam_year");
        $current_exam_month_id = Config::get("global.current_checklist_exam_month");
    ...
    public function examcentersubjectscount(Request $request, $type = "xlsx")
    {
        $request->validate([
            'course' => 'required',
            'stream' => 'required'
        ]);
        $obj = new CenterCountExlExport($request);
        $filename = 'examcentersubjectscount_' . date("d-m-Y-h-i-s") . '.' . $type;
    ...
    {
        $title = "AI Center subject wise NR";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
		...
		$yes_nos = array("1" => "Is Eligible", "2" => "Lock submited feed payed","3" => "Only Lock submited");
		...
		$booklearningtype = $this->master_details($combo_name);
       
		
        return view('examination_reports.aicodewisesubjectsdatastudents', compact('courses', 'fresh_supp_options', 'stream_id', 'title', 'breadcrumbs', 'developeradminrole', 'user_role', 'district_list', 'empty', 'midium', 'yes_no','booklearningtype'));
    }
    ...
    public function downloadaicodewisesubjectsdatastudents(Request $request, $type = "xlsx")
    {
        ...
        $obj = new AicentersubjectCountExlExport($request);
        $filename = 'Aicodesubjectswise studentdata.' . $type;
        Excel::store($obj, $filename);
        return Excel::download($obj, $filename);
    ...
    public function fixcodegenerate($stream = null)
    {
        ...
        ini_set('memory_limit', '10000M');
        ini_set('max_execution_time', '0');
        $aicode_conditions['student_allotments.exam_year'] = Config::get("global.current_materials_exam_year");;
        ...
        if (!empty($aicodemaster)) {
            ...
            foreach ($aicodemaster as $k => $starr) {
                ...
                $conditions = array();
                $conditions['exam_year'] = Config::get('global.current_materials_exam_year');
                ...
                if (!empty($studentarr)) {
                    ...
                    foreach ($studentarr as $stenkey => $stenval) {
                        ...
                        $fixconditions['student_allotments.exam_year'] = Config::get('global.current_materials_exam_year');
    ...
    public function downloadaicodewisesubjectsdatastudentsmediumtype(Request $request, $type = "xlsx")
    {
        ...
        $obj = new AicentersubjectCountStudentExlExport($request);
        $filename = 'Aicodesubjectswisestudentdata.' . $type;
        Excel::store($obj, $filename);
        return Excel::download($obj, $filename);
    }
    ...
    public function getDownloadnominalrollexamcenterwise(Request $request)
    {
        ...
        $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $request->stream . "/" . $request->course . "/" . $District->name . "/" . $request->ecenter . "/" . $filename);
        if (file_exists($path)) {
            return Response::download($path);
        } else {
            return Redirect::back()->with('error', 'File is not Generated');
    ...
    public function single_exam_center_attendance_roll_pdf_view()
    {
        ...
        $empty = array();
        $developeradminrole = Config::get("global.developer_admin");
        $examcenter = Config::get("global.Examcenter");
        $combo_name = 'course';
        $courses = $this->master_details($combo_name);
        $combo_name = 'stream_id';
        $stream_id = $this->master_details($combo_name);
        ...
        $custom_component_obj = new CustomComponent;
        $master = $custom_component_obj->getExamcenterDataByUserId($user_id);
        if ($user_role == $examcenter) {
            $district_list = District::where('state_id', 6)->where('id', @$master->district_id)->pluck('name', 'id');
        ...
        return view('examination_reports.single_material_reports', compact('path', 'type', 'action', 'courses', 'stream_id', 'title', 'breadcrumbs', 'developeradminrole', 'user_role', 'district_list', 'empty', 'examcenter'));
    }
    ...
    {
        $title = "Practical signature Nominal Roll";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
    ...
    {
        $title = "Theory signature Nominal Roll";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
    ...
    public function aicentercoursewisestudentcount(Request $request)
    {

        $title = "AI Center-Coursewise Student Count ";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
        ...
        return view('examination_reports.aicentercoursewisestudentcount', compact('courses', 'stream_id', 'title', 'breadcrumbs', 'developeradminrole', 'user_role', 'district_list', 'empty', 'midium'));


    }

    public function aicentercoursewisestudentcountexcel(Request $request, $type = "xlsx")
    {

        $request->validate([
            'stream' => 'required',
        ]);
        return Excel::download(new AiCenterCourseWiseStudentCountExlExport($request), 'Aicentercoursewisestudentcountexcel.xlsx');
    }
    ...
    public function downloadTocCheckListsallaicenterwisePdf($course = null, $stream = null)
    {
        ...
        $master = Student::where($conditions)
            ->with('application', 'toc', 'toc_subject')->whereRelation('application', $conditions_applications)->orderBy(DB::raw('CAST( IF( LEFT(ai_code,1)=0 , SUBSTR(ai_code,2),ai_code)  AS UNSIGNED)', 'ASC'))->orderBy('enrollment', 'ASC')->get();
    ...
    public function downloadaicodewisesubjectsdatastudentsSupp(Request $request, $type = "xlsx")
    {
        ...
        return Excel::download(new AicentersubjectCountsanssuppExlExport($request), 'Aicodesubjectswisestudentdata.xlsx');
    }


    public function districtwisecategorycount()
    {

        $title = "District wise Category Count";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
        ...
        return view('examination_reports.districtwisecategorycount', compact('courses', 'stream_id', 'title', 'breadcrumbs', 'developeradminrole', 'user_role', 'district_list', 'empty', 'midium'));
    }


    //public function downloaddistrictwisecategorycount(Request $request,$type="xlsx"){
    ...
    // return Excel::download(new DownloaddistrictwisecategoryExlExport($request), 'Downloaddistrictwisecategory.xlsx');
    //}

    public function student_org_fees(Request $request)
    {
        $user_role = Session::get('role_id');
        $combo_name = 'gender';
        $gender_id = $this->master_details($combo_name);
        ...
        $tableData = array(
            ...
            array(
                "lbl" => "Registration Fees",
                'fld' => 'org_registration_fees',
                'input_type' => 'text',
        ...
        Session::put($formId . '_condtions', $condtions);
        $master = $custom_component_obj->getStudentOrgFees($formId, true);
        return view('examination_reports.student_fees_reports', compact('tableData', 'master', 'aiCenters', 'exportBtn', 'formId', 'table_id', 'filters', 'title', 'breadcrumbs', 'gender_id', 'yes_no', 'midium'))->withInput($request->all());
    }
    ...
    public function hall_ticket_api_download(Request $request)
    {
        ...
        $allowips = $custom_component_obj->hallticketAllowIps();
        ...
        $type = 2;
        //@$type = Crypt::decrypt($inputs->type);
        /* Secure Token check end */

        if (@$type == 2) {
            $enrollments = Crypt::decrypt(@$inputs['enrollment']);

            $studentFields = array('students.id', 'students.enrollment');
            $student = Student::where('students.enrollment', $enrollments)->first($studentFields);
            ...
            $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
            //dd($path);
            $filename = 'hallticket_' . $enrollments . '.pdf';
            @$completepath = $path . $filename;

            //echo ($enrollments);die;
            if (File::exists($completepath)) {
                return Response::download($completepath);
            } else {
                $enrollments = Crypt::decrypt($inputs['enrollment']);
                $stream = Config::get("global.current_materials_exam_month");
                $exam_monthyear = Config::get("global.current_materials_exam_year");
                $finedstudentallotment = StudentAllotment::where('enrollment', $enrollments)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->first(['course', 'ai_code']);
                if (empty($finedstudentallotment)) {
                    return Redirect::back()->with('error', 'Enrollment not Found');
                }
                @$course = $finedstudentallotment->course;
                ...
                //$subjects10 = DB::table('subjects')->where(array('deleted'=>0))->where(array('course'=>10))->orderBy('subject_code')->pluck('name','id');
                //$subjects12 = DB::table('subjects')->where(array('deleted'=>0))->where(array('course'=>12))->orderBy('subject_code')->pluck('name','id');
                //$practicalsubjects124 = DB::table('subjects')->where(array('deleted'=>0))->where(array('practical_type'=>1))->orderBy('subject_code')
                //->pluck('subject_code','id')->toArray();
                ...
                if (isset($ai_code) && !empty($ai_code)) {
                    ...
                    $conditions [] = ['student_allotments.supplementary', '=', 0];
                    $conditions [] = ['students.is_eligible', '=', 1];
                    $conditions [] = ['student_allotments.enrollment', '=', $enrollments];
                    $conditions [] = ['student_allotments.exam_year', '=', $exam_monthyear];
                    $conditions [] = ['student_allotments.exam_month', '=', $stream];
                } else {
                    return redirect()->route('hall_ticket_form')->with('error', 'Oop"s! Did you really think you are allowed to see that?');
                ...
                $conditionSupplementary [] = ['student_allotments.exam_month', '=', $stream];
                $conditionSupplementary [] = ['supplementaries.is_eligible', '=', 1];
                $conditionSupplementary [] = ['student_allotments.enrollment', '=', $enrollments];
                $conditionSupplementary [] = ['supplementaries.exam_year', '=', $exam_monthyear];
                $conditionSupplementary [] = ['supplementaries.exam_month', '=', $stream];
                $suppStudents = array();
                $suppStudents = StudentAllotment::select('students.id', 'students.ai_code', 'students.enrollment', 'students.name', 'students.father_name', 'student_allotments.student_id',
                ...
                $studentDocumentPath = $student_document_path[1];
                $students = $dataSave;
                $halltickectqrcode = $this->hallticketqrcode($enrollments);
                //return view('examination_reports.hall_ticket_view',compact('current_year','stream','subjects','subjects10','studentDocumentPath','subjects12','practicalsubjects12','subject_list','categorya','students','subreportname','course','exam_session','exam_time_table_start_end_time'));
                $pdf = PDF::loadView('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'subjects', 'studentDocumentPath', 'practicalsubjects12', 'categorya', 'students', 'subreportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
                $pdf->setTimeout(2000);
                $enrollmentname = 'enrollment';
                $path = public_path("files/reports/" . $current_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
                File::makeDirectory($path, $mode = 0777, true, true);
    ...
    public function hall_ticket_api_downloadold(Request $request)
    {
        ...
        if (@$type == 1) {
            ...
            $student = Student::where('enrollment', $enrollments)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->first();
            $ai_code = @$student->ai_code;
            $enrollment = @$student->enrollment;
            $finedstudentallotment = StudentAllotment::where('ai_code', $ai_code)->where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->first('course');
            if (empty($finedstudentallotment)) {
                return Redirect::back()->with('error', 'Enrollment not Found');
            }
            $course = $finedstudentallotment->course;
            ...
            foreach ($aiCenters as $key => $value) {
                ...
                $students = $dataSave;
                $halltickectqrcode = $this->hallticketqrcode($enrollment);

                return view('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'subjects', 'subjects10', 'studentDocumentPath', 'subjects12', 'practicalsubjects12', 'subject_list', 'categorya', 'aicode', 'students', 'subreportname', 'reportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
    ...
    public function hall_ticket_api_bulk_ganrate(Request $request)
    {
        ini_set('memory_limit', '3000M');
        ini_set('max_execution_time', '0');
        $stream = Config::get("global.current_materials_exam_month");
        $exam_monthyear = Config::get("global.current_materials_exam_year");
        $finedstudentallotment = StudentAllotment::where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->get(['course', 'enrollment', 'ai_code']);
        foreach ($finedstudentallotment as $key => $value) {
            ...
            foreach ($aiCenters as $key => $value) {
                ...
                $pdf = PDF::loadView('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'subjects', 'subjects10', 'studentDocumentPath', 'subjects12', 'practicalsubjects12', 'subject_list', 'categorya', 'aicode', 'students', 'subreportname', 'reportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
                $pdf->setTimeout(2000);
                $enrollmentname = 'enrollment';
                $path = public_path("files/reports/" . $current_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
                File::makeDirectory($path, $mode = 0777, true, true);
    ...
    public function hall_ticket_single_bulk_generate(Request $request)
    {
        ini_set('memory_limit', '3000M');
        ini_set('max_execution_time', '0');
        $stream = Config::get("global.current_materials_exam_month");
        ...
        foreach ($finedstudentallotment as $key => $value) {
            ...
            $students = $dataSave;
            $halltickectqrcode = $this->hallticketqrcode($enrollment);
            $pdf = PDF::loadView('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'studentDocumentPath', 'practicalsubjects12', 'subjects', 'categorya', 'students', 'subreportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
            $pdf->setTimeout(2000);
            $enrollmentname = 'enrollment';
            $path = public_path("files/reports/" . $current_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
            File::makeDirectory($path, $mode = 0777, true, true);
    ...
    public function hall_ticket_single_bulk_generateold(Request $request)
    {
        ini_set('memory_limit', '3000M');
        ini_set('max_execution_time', '0');
        $stream = Config::get("global.current_materials_exam_month");
        $exam_monthyear = Config::get("global.current_materials_exam_year");
        $finedstudentallotment = StudentAllotment::where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->limit(12)->get(['course', 'enrollment', 'ai_code']);
        foreach ($finedstudentallotment as $key => $value) {
            $ai_code = @$value->ai_code;
            $course = @$value->course;
            $enrollment = @$value->enrollment;
            ...
            $students = $dataSave;
            $halltickectqrcode = $this->hallticketqrcode($enrollment);
            $pdf = PDF::loadView('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'subjects', 'subjects10', 'studentDocumentPath', 'subjects12', 'practicalsubjects12', 'subject_list', 'categorya', 'aicode', 'students', 'subreportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
            $pdf->setTimeout(2000);
            $enrollmentname = 'enrollment';
            $path = public_path("files/reports/" . $current_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/");
            File::makeDirectory($path, $mode = 0777, true, true);
    ...
    public function AdmitCardDownloadForAll(Request $request)
    {
        $custom_component_obj = new CustomComponent;
        $resultCheckStatus = $custom_component_obj->getIsAllowToShowAdmitCardDownloadForAll();
        if (@$resultCheckStatus) {
        } else {
            return redirect()->route("landing")->with('error', 'Admit Card Not published.');
        }

        $title = "Download Admit(HallTicket) Card";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
        ...
        if (count($request->all()) > 0) {
            ...
            return redirect()->route('downloadAdmitCard', encrypt($inputs['enrollment']));


        }
        $captchaImage = $custom_component_obj->generateCaptcha(1, 20, 150, 30);
        return view('examination_reports.downloadAdmitCard', compact('captchaImage', 'title', 'breadcrumbs'));
    }

    public function downloadAdmitCard($enrollment = null)
    {
        $enrollments = Crypt::decrypt($enrollment);
        $enrollmentname = 'enrollment';
        $exam_year = Config::get("global.current_materials_exam_year");
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($exam_year);
        $stream = Config::get("global.current_materials_exam_month");
        $aicentermaterial = "aicentermaterial";
        $completepath = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/") . 'hallticket_' . $enrollments . '.pdf';
        if (File::exists($completepath)) {
            return Response::download($completepath);
        } else {
            $enrollments = Crypt::decrypt($enrollment);
    ...
    public function hallticketsinglegeneratesuperadminenrollment(Request $request)
    {
        ...
        $finedstudentallotment = StudentAllotment::where($conditions)->whereNull('deleted_at')->first(['course', 'ai_code']);
        if (empty($finedstudentallotment)) {
            return Redirect::back()->with('error', 'Enrollment not Found');
        }
        @$course = $finedstudentallotment->course;
    ...
    public function hall_ticket_single_bulk_aicode_wise_generate(Request $request)
    {
        ini_set('memory_limit', '3000M');
        ini_set('max_execution_time', '0');
        $stream = Config::get("global.current_materials_exam_month");
        $exam_monthyear = Config::get("global.current_materials_exam_year");
        $finedstudentaicode = StudentAllotment::where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->groupBy('ai_code')->pluck('ai_code', 'ai_code');
        if (count($request->all()) > 0) {
            $finedstudentallotment = StudentAllotment::where('ai_code', @$request->aicode)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->get(['course', 'enrollment', 'ai_code']);
            foreach ($finedstudentallotment as $key => $value) {
                $ai_code = @$value->ai_code;
                $course = @$value->course;
                $enrollment = @$value->enrollment;
        ...
        return view('examination_reports.aicodewisehalltickect', compact('finedstudentaicode'));

    }

    public function hallticketsinglegeneratesuperadmin($enrollment = null)
    {
        $stream = Config::get("global.current_materials_exam_month");
        $exam_monthyear = Config::get("global.current_materials_exam_year");
        $finedstudentallotment = StudentAllotment::where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->first(['course', 'ai_code']);
        if (empty($finedstudentallotment)) {
            return Redirect::back()->with('error', 'Enrollment not Found');
        }
        @$course = $finedstudentallotment->course;
    ...
    public function getDownloadfixcodezipdownload($course = null, $stream = null)
    {
        $current_admission_year_string = Config::get('global.current_materials_exam_year');
        $exam_month = Config::get('global.current_materials_exam_month');
        ini_set('memory_limit', '3000M');
        ...
        $current_folder_year = $this->getCurrentYearFolderNamematerialschecklist($current_admission_year_string);
        $streamname = 'stream';
        $checklistall = "fixcode";
        $checklist_zip = "fixcode.zip";
        $checklist_login_zip = "fixcodeslogin";
        $nextPath = $current_folder_year . "/" . $checklistall . "/" . $stream . "/" . $course . "";
        $folder_path = public_path("files/reports/" . $nextPath);
        if (File::exists($folder_path)) {
            $destdirTemp = 'allzipsave/' . $checklistall . '/' . $current_admission_year_string . '/' . "stream" . $stream . "/" . $checklist_login_zip . "/";
            $folderPathsTemp = public_path($destdirTemp);
    ...
    public function courseAurMediumWiseBookSubjectsDataStudents(Request $request)
    {
        $title = "Course And Medium Wise Book  Reports";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
        ...
        if (count($request->all()) > 0) {
            ...
            $validator = Validator::make($request->all(), [
                ...
                [
                    ...
                    'midium.required' => 'Medium is required.',
                ]);
            if ($validator->fails()) {
            ...
            if ($request->midium == 1) {
                $medium = 'hindi';
            } elseif ($request->midium == 2) {
                $medium = 'english';
            }

            $obj = new CourseMediumWiseBookSubjectCountExlExport($request);
            $filename = $request->course . $medium . 'bookstudentdata.' . $type;
            Excel::store($obj, $filename);
            return Excel::download($obj, $filename);
        ...
        return view('examination_reports.courseaurmediumwisebooksubjectsdatastudents', compact('courses', 'title', 'breadcrumbs', 'empty', 'midium'));
    }

    public function hallticketqr(Request $request)
    {
        ...
        $enrollmentname = 'enrollment';
        $filename = 'hallticket_' . $enrollment . '.pdf';
        $path = public_path("files/reports/" . $current_folder_year . "/" . $aicentermaterial . "/" . $stream . "/" . $enrollmentname . "/" . $filename);
        //return response()->download($path);
        return redirect()->route('hallticketqrviews', Crypt::encrypt($enrollment));
    }

    public function hallticketqrviews($enrollments = null)
    {
        $enrollment = Crypt::decrypt($enrollments);
        $stream = Config::get("global.current_materials_exam_month");
        $exam_monthyear = Config::get("global.current_materials_exam_year");
        $finedstudentallotment = StudentAllotment::where('enrollment', $enrollment)->where('exam_year', $exam_monthyear)->where('exam_month', $stream)->whereNull('deleted_at')->first(['course', 'ai_code']);
        if (empty($finedstudentallotment)) {
        ...
        $halltickectqrcode = $this->hallticketqrcode($enrollment);
        return view('examination_reports.hall_ticket_view', compact('current_year', 'stream', 'subjects', 'studentDocumentPath', 'practicalsubjects12', 'categorya', 'students', 'subreportname', 'course', 'exam_session', 'exam_time_table_start_end_time', 'halltickectqrcode'));
        //$pdf =  PDF::loadView('examination_reports.hall_ticket_view',compact('current_year','stream','subjects','studentDocumentPath','practicalsubjects12','categorya','students','subreportname','course','exam_session','exam_time_table_start_end_time','halltickectqrcode'));

    }

    public function aicenterwisestudentandfeesdata()
    {
        $title = "AI Center Wise Studnet Fees Data ";
        $breadcrumbs = array(
            array(
                "label" => "Dashboard",
                'url' => ''
        ...
        $combo_name = 'stream_id';
        $stream_id = $this->master_details($combo_name);
        return view('examination_reports.aicenterwisestudentandfeesdata', compact('breadcrumbs', 'title', 'admission_sessions', 'stream_id'));
    }


    public function aicenterwisestudentandfeesdataexcel(Request $request, $type = "xlsx")
    {
        $request->validate([
            'exam_year' => 'required',
            'stream' => 'required'
        ...
        $filenames = $exam_month[$request->stream] . "_" . $admission_sessions[$request->exam_year];
        $obj = new AICenterStudentFeesCountExlExport($request);
        $filename = $filenames . '_' . date("d-m-Y-h-i-s") . '.' . $type;
        Excel::store($obj, $filename);
        return Excel::download($obj, $filename);
    }
