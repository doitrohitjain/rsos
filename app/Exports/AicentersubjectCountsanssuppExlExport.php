...
class AicentersubjectCountsanssuppExlExport implements FromCollection,WithHeadings
{
    /**
    ...
    */
	...
	public function collection(){
		...
		SUM(CASE WHEN es.subject_id=41 THEN 1 ELSE 0 END) as Sub_208
	     FROM rs_students s
			INNER JOIN rs_aicenter_details ad ON s.ai_code = ad.ai_code
			INNER JOIN rs_exam_subjects es ON es.student_id= s.id 
			WHERE
			s.exam_month = '$stream'
			AND s.is_eligible = 1
		...
		SUM(CASE WHEN ss.subject_id=41 THEN 1 ELSE 0 END) as Sub_208
	     FROM rs_supplementaries s
			INNER JOIN rs_aicenter_details ad ON s.ai_code = ad.ai_code
			INNER JOIN rs_supplementary_subjects ss ON ss.supplementary_id= s.id 
			WHERE
			s.exam_month = '$stream'
			AND s.is_eligible = 1
			AND s.exam_year = '$examyear'
